...
/*
 Binds the type VectorInt in javascript to be used as a std::Vector<int>
*/
...
void Goban::populateBoard(std::vector<int> boardSchema, int boardSize) {
...
/*
   std::vector<int> vec;
    for(int j = 0; j < 361; j++) {
      vec.push_back(boardSchema.at(j));
    }
*/
/*
      switch (a) {
...
/*
      ...
      printf("\n");
*/
      //std::cout<<a<<", "<<b <<": "<<schema[i]<<std::endl;
...
*/
    ...
    for (int i=0; i < schema.size(); i++) {
            y = i % schema.size();
            x = i / schema.size();
            this->board[y][x] = schema[i];
            std::cout<<y<<", "<<x <<": "<<schema[i]<<std::endl;
            //x = Math.floor(i / game.size); // need function to round down
    }
    */
}
...
Console instructions:
  ...
  var arr = new Module.VectorInt;
  for(var i = 0; i<361; i++) { arr.push_back(1); }
  instance.populateBoard(arr, 19);
  instance.print();
